...
public class VideoDetailActivity extends BaseActivity {
    ...
    private void setupGameDropdown() {
        // Load games via gateway instead of direct Firebase read
        com.genzopia.Instagame.gateway.GatewayClient.INSTANCE.getCallApi()
                .getChannelGames(currentUserId)
    ...
    private void loadVideoData() {
        // Load video via gateway channel videos endpoint
        com.genzopia.Instagame.gateway.GatewayClient.INSTANCE.getCallApi()
                .getChannelVideos(currentUserId)
                .enqueue(new retrofit2.Callback<com.genzopia.Instagame.gateway.ReelsPageResponse>() {
                    ...
                    public void onResponse(@NonNull retrofit2.Call<com.genzopia.Instagame.gateway.ReelsPageResponse> call,
                        ...
                        com.genzopia.Instagame.gateway.ReelDTO video = null;
                        for (com.genzopia.Instagame.gateway.ReelDTO r : resp.body().getData()) {
                            if (r.getVideoId().equals(videoId)) { video = r; break; }
                        }
                        if (video == null) { Toast.makeText(VideoDetailActivity.this, "Video not found", Toast.LENGTH_SHORT).show(); finish(); return; }

                        isOwnVideo = currentUserId.equals(video.getDeveloperId());
                        originalTitle = video.getTitle();
                        originalDescription = video.getDescription();
                        originalGameId = video.getGameId();
                        originalGameName = video.getGameName();

                        titleInput.setText(originalTitle);
                        descriptionInput.setText(originalDescription);
                        viewCountText.setText(video.getViewCount() + " views");
                        likeCountText.setText(video.getLikeCount() + " likes");
                        shareCountText.setText("0 shares");

                        if (!originalGameName.isEmpty()) {
                            gameDropdown.setText(originalGameName);
                            gameTagChipGroup.removeAllViews();
                            Chip gameChip = new Chip(VideoDetailActivity.this);
                            gameChip.setText(originalGameName);
                            gameChip.setChipBackgroundColorResource(R.color.button_primary);
                    ...
                    @Override
                    public void onFailure(@NonNull retrofit2.Call<com.genzopia.Instagame.gateway.ReelsPageResponse> call, @NonNull Throwable t) {
                        Toast.makeText(VideoDetailActivity.this, "Failed to load video data", Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    private void loadGameName(String gameId) {
        // Game name already populated in loadVideoData from gateway response
    }
    ...
    private void saveVideoChanges() {
        ...
        // Update via gateway — server verifies ownership (Req security)
        final String resolvedGameId = newGameId;
        java.util.Map<String, String> updates = new java.util.HashMap<>();
        updates.put("video_title", newTitle);
        updates.put("description", newDescription);
        updates.put("game_id", resolvedGameId);

        com.genzopia.Instagame.gateway.GatewayClient.INSTANCE.getCallApi()
                .updateVideo(videoId, updates)
                .enqueue(new retrofit2.Callback<Void>() {
                    @Override
                    public void onResponse(@NonNull retrofit2.Call<Void> call, @NonNull retrofit2.Response<Void> resp) {
                        if (resp.isSuccessful()) {
                            Toast.makeText(VideoDetailActivity.this, "Video updated successfully", Toast.LENGTH_SHORT).show();
                            originalTitle = newTitle;
                            originalDescription = newDescription;
                            originalGameName = newGameInput;
                            originalGameId = resolvedGameId;
                            Intent resultIntent = new Intent();
                            resultIntent.putExtra("video_id", videoId);
                            setResult(RESULT_VIDEO_UPDATED, resultIntent);
                            checkForChanges();
                        } else {
                            Toast.makeText(VideoDetailActivity.this, "Failed to update video: HTTP " + resp.code(), Toast.LENGTH_SHORT).show();
                        }
                    }
                    @Override
                    public void onFailure(@NonNull retrofit2.Call<Void> call, @NonNull Throwable t) {
                        Toast.makeText(VideoDetailActivity.this, "Failed to update video: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    private void deleteVideo() {
        androidx.appcompat.app.AlertDialog loadingDialog = new androidx.appcompat.app.AlertDialog.Builder(this)
                .setView(R.layout.loading_dialog).setCancelable(false).create();
        loadingDialog.show();

        // Delete via gateway — server verifies ownership before deleting
        com.genzopia.Instagame.gateway.GatewayClient.INSTANCE.getCallApi()
                .deleteVideo(videoId)
                .enqueue(new retrofit2.Callback<Void>() {
                    @Override
                    public void onResponse(@NonNull retrofit2.Call<Void> call, @NonNull retrofit2.Response<Void> resp) {
                        ...
                        if (resp.isSuccessful()) {
                            ...
                            prefs.edit().putString("deleted_video_id", videoId).apply();
                            Intent resultIntent = new Intent();
                            resultIntent.putExtra("deleted_video_id", videoId);
                            setResult(RESULT_VIDEO_DELETED, resultIntent);
                            finish();
                        } else {
                            Toast.makeText(VideoDetailActivity.this, "Failed to delete video: HTTP " + resp.code(), Toast.LENGTH_SHORT).show();
                        }
                    }
                    @Override
                    public void onFailure(@NonNull retrofit2.Call<Void> call, @NonNull Throwable t) {
                        if (loadingDialog.isShowing()) loadingDialog.dismiss();
                        Toast.makeText(VideoDetailActivity.this, "Failed to delete video: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
